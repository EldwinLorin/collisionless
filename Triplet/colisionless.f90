...
do j=1,nbins
 ...
 !   Allocate(denominator(nbins),frac(nbins), from_CCH2(nbins), from_HCCH(nbins), den_1(nbins), den_2(nbins))
 ...
 write(100,'(A5, A15, A15, A15)') 'J', 'k(T,J)', 'kHCCH(T,J)', 'kback(T,J)'
    ...
    do k=1, nJ
        ...
        do i=1, nbins
            if ((energy(i) .gt. product_energy(1) )) then

        den_1=(sumstates(1,i,k)+sumstates(2,i,k))/sumstates(2,i,k)
        den_2=(sumstates(3,i,k)*sumstates(3,i,k))/(sumstates(4,i,k)+sumstates(3,i,k))
        den_2=den_2-(sumstates(3,i,k)+sumstates(2,i,k))

        denominator=den_1*den_2
        denominator=denominator+sumstates(2,i,k)
        from_HCCH=sumstates(1,i,k)*((sumstates(3,i,k)*sumstates(4,i,k))/(sumstates(4,i,k)+sumstates(3,i,k)))*(-1)/denominator

        frac=sumstates(1,i,k)

        frac=frac*deltaE*exp(-(energy(i)-product_energy(1))/(kb*T))/(kb*T)
        from_HCCH = from_HCCH * deltaE * exp(-(energy(i) - product_energy(1)) / (kb*T)) / (kb*T)
        ...
        from_HCCH=0
            ...
            integral=integral+frac*(2*k+1)
            integral_HCCH=integral_HCCH+from_HCCH*(2*k+1)
    end do
...
!    kTJ_CCH2(k)=integral_CCH2*E_independant_factor
    ...
    write(100,'(I5, 1PE15.5, 1PE15.5, 1PE15.5)') k, kTJ(k), kTJ_HCCH(k), kTJ(k)-kTJ_HCCH(k)
end do
...
write(200,'(A, T50, 1PE15.7)') 'kprod(T)=', rateconst_HCCH
!write(200,'(A, T50, 1PE15.7)') 'kCCH2(T)=', rateconst_CCH2
write(200,'(A, T50, 1PE15.7)') 'kHCCH(T)=', rateconst_HCCH
write(200,'(A, T50, 1PE15.7)') 'kback(T)=', rateconst - rateconst_HCCH
write(200,'(A, T50, F15.7)') 'Branching ratio for products=', rateconst_HCCH/rateconst
write(200,'(A, T50, F15.7)') 'Branching ratio for backdissociation=', 1-(rateconst_HCCH/rateconst)
...
write(300,'(1PE15.7)') rateconst_HCCH
write(300,'(1PE15.7)') rateconst - rateconst_HCCH
write(300,'(F15.7)') rateconst_HCCH/rateconst
write(300,'(F15.7)') 1-(rateconst_HCCH/rateconst)
...
!    deallocate(denominator, den_1,den_2)
